> **来源**：AutoJs6 官方文档 `https://docs.autojs6.com/#/automator`（离线存档，正文与官网逐字一致，未删改）。
> **存档版本**：2026-09-23 抓取，接口集对应 **AutoJs6 6.8.0**。
> **本文件里最常用的两节**（其余是控件选择器 `UiSelector`/`UiObject`、无障碍服务、`Toolkit` 工具集）：
> - **「基于坐标的触摸模拟」与「安卓 7.0 以上的触摸和手势模拟」**（约 `### click` 起 ~600 行）——
>   `click` / `longClick` / `press(x,y,duration)` / `swipe(x1,y1,x2,y2,duration)` /
>   `gesture(duration, [x,y], ...)` / `gestureAsync` / `gestures([delay,duration,[x,y],...], ...)` / `gesturesAsync`。
>   **这组是摇杆与吐孢/分身动作的全部原语。参数类型与阻塞语义务必照抄本文件，不要凭记忆写。**
> - **`setScreenMetrics(width, height)`** —— 坐标缩放基准。**坐标整体偏移 / 摇杆跑到屏幕角上 / 点击落在错误位置时，第一个要查的就是它**（详见 `references/AI_AutoJS编码强制规范.md` 与 `故障速查.md`）。
> **读法**：本文件约 8.9 万字符，**严禁通读**。先 Grep 函数名定位，再 offset/limit 只读命中那段（成本纪律见 `references/执行手册.md` §四.4）。
