...
10. 临时文件放 `<skill_dir>/temp`，但**只放用完即删的**；会被复用 / 被文档引用 / 作硬结论证据的脚本放正式位置（PC 侧复核件与手机侧探针 → `scripts/verify/<域>/`，判据见 `references/目录架构与路径约定.md` §二）——`temp/` 会被整目录清理，放进去等于断链
11. **本技能文档是自进化的（活文档）——只留正确的知识**。**总原则（2026-09-26 用户定则）：文档只做参考，实践是检验真理的唯一标准**——实际效果对，才是真的对；实测效果与文档说法冲突，说明文档是错的，必须以实际效果为准处理实际、并当场订正文档，**不许拿文档当依据硬扛实测结果**。任何时候（尤其在下发实操、真机验证、查证官方文档的过程中），一旦拿到**硬证据**证明某条知识与实测结果/官方文档不符，就**当场删除它、写入已验证的**。错误知识**一律删干净**——不并存、不标注、不归档（连"曾记录 X、已推翻"这类痕迹也不留）。⛔ 不许"留着但知道它是错的"，也不许只在本次会话里绕过；想查历史去翻 git。
...
13. **⛔ 写或改 AutoJS UI 界面 / 悬浮窗前，必须先完整读一遍 `references/AutoJS6_UI界面与悬浮窗XML指南.md`**——插值、尺寸单位、layout_weight、控件选型、button 高度坑、悬浮窗专项全在里面，不读就写=盲写（2026-09-26 实证：未读先写悬浮窗 → dp/px 混用 + 按钮文字被裁，返工 3 轮才对）。**改一行 UI 也算改**，同样先读；读完仍以实测效果为验收标准（见约束 11 总原则）。
