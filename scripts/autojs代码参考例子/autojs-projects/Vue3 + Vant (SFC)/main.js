...
// 处理来自 Web 的请求（网页侧用 $autojs.invoke('名称', 参数) 调用，
// 返回普通值 → 网页 then 拿到；返回 Promise → 异步 resolve 后回推）
web.jsBridge
    // 处理读取本地文件的请求（vue3-sfc-loader 的 getFile 走这里，读 web/ 下的 .vue / .js）
    .handle('fetch', (event, args) => {
        return files.read(files.join(files.path('web'), args.path));
    })
    // 处理获取设备信息的请求 —— 演示「handle 返回对象 → 网页 invoke 拿数据」
    .handle('get-device-info', (event) => {
        return {
            model: device.model,
            brand: device.brand,
